...
거래소 화폐는 임의로 무한 예치할 수 없으며, 아래 조건에 따라 획득/예치가 가능해야 합니다.
구체적인 수치는 게임 밸런스에 맞게 추후 결정이 필요합니다.

### 예치 가능 조건 (미결정 — 하나 이상 선택)

| 방식 | 설명 | 고려사항 |
|------|------|---------|
| 점수 기반 | 게임 점수를 일정 비율로 화폐로 전환 | 점수 위변조 방지 필요, 전환 비율 결정 필요 |
| 레벨 기반 | 일정 레벨 달성 시 지급 | 레벨업 조건 정의 필요 |
| 판매 수익 순환 | 아이템 판매로 얻은 수익만 사용 | 최초 화폐 유입 경로 별도 필요 |
| 일일 한도 | 하루 예치 가능 금액 상한 설정 | 서버에서 날짜 기준 누적 검증 필요 |

### 예치 시 서버 검증 항목 (추후 조건 확정 후 구현)

- 예치 조건 충족 여부 확인 (점수/레벨 등 근거 데이터 첨부 필요)
- 1회 최소/최대 예치 금액 범위 제한
- 일일 누적 예치 한도 초과 여부 확인
- 조건 미충족 시 예치 거부 및 사유 반환

> 조건 없이 자유 예치를 허용하면 화폐 인플레이션 및 거래 불균형이 발생할 수 있으므로
> 반드시 하나 이상의 조건을 확정한 후 구현해야 합니다.
